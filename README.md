...
6. DB연결
    ...
    6. DB데이터어댑터(`SqlDataAdapter`) : 연결이후 데이터처리를 쉽게 도와주는 객체
        ...
        - 거의 SELECT문에서만 사용
    7. DB데이터리더(`SqlDataReader`)/데이터어댑터(SqlDataAdapter)
...
- MVC : Model-View-Controller 패턴
    ...
    - Spring Boot, `ASP.NET`, jDango 등 웹개발 아케텍처패턴으로 표준으로 사용
...
- MVVM 장단점
    ...
    - 로직 분리로 구조가 명확해짐. 자기할일만 하면 됌
    - 팀으로 개발시 역할분담이 학실. 팀프로젝트에 알맞음
    - 테스트와 유지보수는 쉬움
    - 구조가 복잡. 디버깅이 어려움
    - 스케일이 커짐
...
- 종류
    ...
    - `MVVVM Light Toolkit` : 가장 가벼운 MVVM 입문용. 쉬운 Command 지원. 개발종료.
        - 확장성이 떨어지고
...
3. Models, View, ViewModels로 폴더 생성
...
2. 새항목 > 데이터 >  XML파일 > NLog.config 수정
3. Info < `Debug` < Warn < Error < Gatal
