...
PUB_ID = 'IOT68' # 본인 아이피 마지막주소
BROKER = '210.119.12.68' # 본인 아이피
PORT = 1883
TOPIC = 'smarthome/68/topic'    # publish/subscribe에서 사용할 토픽(핵심)
COLORS = ['RED', 'ORNGE', 'YELLOW', 'GREEN', 'BLUE', 'NAVY', 'PURPLE']
COUNT = 0
...
try:
    ...
    while True:
        ...
        detect = random.uniform(0,1) # [Fake] 인체센서, 실제로는 센서에서 값을 받아옴
        photo = random.uniform(50,255)  # [Fake] 포토센서, 255쪽이 어두움 센싱
        ...
        raw_data['SENSING_DT'] = currtime.strftime(f"%Y-%m-%d, %H:%M:%S") # C# 'yyyy-
